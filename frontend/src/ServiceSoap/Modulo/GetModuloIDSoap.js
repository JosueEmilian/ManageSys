async function GetModuloIDSoap(id) {
  try {
    const xml = `<Envelope xmlns="http://schemas.xmlsoap.org/soap/envelope/">
            <Body>
                <obtenerModuloPorID xmlns="http://service/">
                    <id xmlns="">${id}</id>
                </obtenerModuloPorID>
            </Body>
        </Envelope>`;
    ...
    const getModuloID = Array.from(xmlDoc.getElementsByTagName("return")).map(
      (node) => {
...
export default GetModuloIDSoap;
